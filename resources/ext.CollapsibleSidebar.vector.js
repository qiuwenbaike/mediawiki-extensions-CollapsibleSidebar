...
(function () {
	...
	icon.id = 'sidebarCollapse';
	icon.src = IS_COLLAPSED ? NEXT_ICON : PREV_ICON;
	icon.draggable = false;
	icon.alt = IS_COLLAPSED ? message('show-link') : message('hide-link');
	icon.title = IS_COLLAPSED ? message('show-link-tooltip') : message('hide-link-tooltip');
	icon.style.left = IS_COLLAPSED ? '0.3em' : (BODY_WIDTH() < 982 ? '9.3em' : '10.3em');
	document.body.appendChild(icon);

	var iconLink = document.createElement('a');
	iconLink.href = WIKI_LOGO.href;
	iconLink.title = WIKI_LOGO.title;

	var logo = document.createElement('img');
	logo.id = 'sliderCollapseLogo';
	logo.classList.add('mw-no-invert');
	logo.src = LOGO_ICON;
	logo.alt = WIKI_LOGO.title;
	logo.title = WIKI_LOGO.title;
	logo.style.display = IS_COLLAPSED ? '' : 'none';

	iconLink.appendChild(logo);
	document.getElementById('mw-navigation').appendChild(iconLink);
	...
	var switchMode = {
		...
		hide: function () {
			...
			document.getElementById('sidebarCollapse').src = NEXT_ICON;
		...
		show: function () {
			...
			document.getElementById('sidebarCollapse').src = PREV_ICON;
			document.getElementById('sidebarCollapse').style.left =
				(switchMode.BODY_WIDTH >= 982) ? '10.3em' : '9.3em';
		}
	};

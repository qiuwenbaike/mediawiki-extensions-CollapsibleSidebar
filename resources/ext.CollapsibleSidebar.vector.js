'use strict';
( ( $ ) => {
	const GADGET_NAME = 'ext.gadget.CollapsibleSidebar';
	const images = {
	...
	const updatePositionHelper = ( positionLeftObject ) => {
		for ( const [ item, positionLeft ] of Object.entries( positionLeftObject ) ) {
			...
			const currentPositionLeft = $element.css( 'left' );
	...
	const collapsibleSidebarMain = ( observer ) => {
		observer?.disconnect();
		const $sidebarCollapseButton = $( '<img>' )
			.attr( {
				id: 'sidebarCollapse',
				src: images.prev,
				alt: mw.message( 'collapsiblesidebar-collapse-link' ),
		...
		const $sliderCollapseLogo = $( '<img>' )
			.addClass( 'mw-no-invert' )
			.attr( {
				id: 'sliderCollapseLogo',
				src: images.logo,
				alt: mw.message( 'collapsiblesidebar-show-link' ),
				title: mw.message( 'collapsiblesidebar-show-link-tooltip' )
			} )
			.css( 'display', 'none' );
		...
		$sidebarCollapseButton
			...
			.on( 'mouseout mouseover', function ( { type } ) {
